...
public class SpkResponseModel {
    ...
    private ArrayList<DetailSpkProses> getProgress(Set<SpkProses> spkProseses) {
        ...
        temp.sort(Comparator.comparingInt(SpkProses::getOrder1));

        temp.forEach(spkProses -> {
            String namaProses = spkProses.getProses().getNama();
            //TODO: implement date
            DetailSpkProses detailSpkProses = new DetailSpkProses(getCatatans(spkProses.getCatatans()), new Date(), namaProses, spkProses.getId(), spkProses.getTanggalSelesai(), spkProses.getDurasiNyata());
    ...
    private ArrayList<DetailVarianRes> getVarians(Set<SpkVarian> spkVarians) {
        ...
        spkVarians.forEach(spkVarian -> {
            ...
            DetailVarianRes varian = new DetailVarianRes(spkVarian.getPanjangKain().getPanjang(), spkVarian.getPanjangKain().getSatuanPanjangKain().getNama(), getUkuranJumlahs(spkVarian));
            results.add(varian);
        });
        return results;
    }
    ...
    private HashSet<DetailUkuranJumlahRes> getUkuranJumlahs(SpkVarian spkVarian){
        ...
        spkVarian.getUkuranJumlahs().forEach(ukuranJumlah -> {
            DetailUkuranJumlahRes detailUkuranJumlah1 = new DetailUkuranJumlahRes(ukuranJumlah.getUkuran().getNama(), ukuranJumlah.getJumlah());
