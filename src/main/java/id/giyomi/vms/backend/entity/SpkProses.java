...
@Entity
public class SpkProses extends AuditModel<User> {
    ...
    @JsonIgnore
    @OneToMany( targetEntity= Catatan.class, fetch = FetchType.EAGER )
    @JoinColumn(name = "spk_proses_id")
    private Set<Catatan> catatans = new HashSet<>();

    public SpkProses(Integer durasiRencana, Proses proses, Spk spk, Integer order) {
    ...
    public void setOrder1(Integer order) {
        this.order1 = order;
    }
